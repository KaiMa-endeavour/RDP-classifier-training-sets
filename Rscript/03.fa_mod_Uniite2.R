...
fa <- read_fasta('/clusterfs/node1/makai/Database/Unite2/Unite2.fasta')
nworker <- 62
...
header <- gsub('\tr__root;', ' ', header)
header <- word(header, 1, -2, sep = ';')
lineage <- word(header, 2, -1)
...
fwrite(tax, paste0(output_path, 'tax_Unite2.txt'), sep = '\t')
...
write_fasta(fa_fungi, paste0(output_path, 'mod_Unite2.fa'))
